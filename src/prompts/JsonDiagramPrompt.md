...
- All nodes must be assigned exactly one `"kind"` value from this list.
- The `"kind"` determines the visual styling of the node; nodes with the same `"kind"` are styled consistently but must have different labels and metadata when they represent different components.
...
- If a UI calls a service which calls a database, model edges:
  - `ui → service`
  - `service → database`
- Do **not** add `ui → database` unless the UI directly queries the database.
